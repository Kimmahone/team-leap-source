...
/* 섞인 목록에서는 문장 끝의 단서가 앞의 모든 항목을 덮어씌웁니다 —
   울릉만 직선인데 끝에서 「실제 도로」라고 하니 210km 도 도로처럼 읽혔습니다. */
check('항목마다 무엇으로 잰 값인지 값 옆에 바로 적는다',
  /\(직선·육로 없음\)/.test(js));
...
check('지도 시군 딱지는 서로 덮지 않게 비켜 선다', /function mvDeclutter\(map, marks\)/.test(js) && /const bubs = MV\.marks\.filter/.test(js) && /mvDeclutter\(map, bubs\)/.test(js));
